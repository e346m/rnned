...
class LSTMDec(chainer.Chain):
    def __init__(self, in_size, out_size, lateral_init=None, upward_init=None,
                 diagonal_init=None, bias_init=0, forget_bias_init=0):
        super(LSTMDec, self).__init__(
          ...
          lateral=linear.Linear(out_size, 4 * out_size, initialW=0,
                                nobias=True),
        )
    ...
    def __call__(self, prev_y):
        ...
        lstm_in, cfe_rest = batch_process(batch, self.cfe,
                                          self.diagonal, lstm_in)
